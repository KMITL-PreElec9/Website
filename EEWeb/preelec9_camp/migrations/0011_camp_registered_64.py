# Generated by Django 3.2.3 on 2021-06-14 10:37

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('preelec9_camp', '0010_auto_20210614_1737'),
    ]

    operations = [
        migrations.CreateModel(
            name='Camp_Registered_64',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('registered_on', models.DateTimeField(null=True)),
                ('comment', models.CharField(max_length=300, null=True)),
                ('campdata_64', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='preelec9_camp.campdata_64')),
                ('registered_by', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'Camp Registered 64',
                'verbose_name_plural': 'Camp Registered 64',
            },
        ),
    ]
